...
export interface TaskQuery {
  search?: string; cat?: string; status?: 'todo' | 'done'; priority?: Task['priority'];
  sort?: 'createdAt' | 'title' | 'due' | 'priority' | 'status'; order?: 'asc' | 'desc';
}

/* ── GET all tasks ──────────────────────────────────────── */
export async function getTasks(query: TaskQuery = {}, signal?: AbortSignal): Promise<Task[]> {
  const { data } = await axiosInstance.get<Task[]>('/task', { params: query, signal });
  return data;
}
